...
def get_correlations(qtarget, corr_file):
    f = open(corr_file, "r").readlines()
    for line in f:
        target, R_square, p_val1, Sp_corr, p_val2 = line.strip().split("\t")
        if qtarget == target:
            return (R_square, p_val1, Sp_corr, p_val2)
...
def main():
    f = open("616.10kb.scores", "w")
    gbid_to_score = defaultdict()

    corr_file = "616.correlations"
    ...
    for gbid, target in tqdm.tqdm(sorted(gbids_and_targets)):
        print gbid, target
        if get_correlations(target, corr_file) !=None:
            R_square, p_val1,  Sp_corr, p_val2 = get_correlations(target, corr_file)
        else:
            R_square = p_val1 = Sp_corr = p_val2 = -1000
        ...
        f.write("%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n" %
                (gbid, target, target_ubiquity, score,
                 homologs, json.dumps(homologs_list),
                 json.dumps(pident_list),
                 R_square, p_val1, Sp_corr, p_val2))
    f.close()

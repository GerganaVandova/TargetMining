...
f = open("KS.119.10kb.fasta.descr", "w")
...
for record in SeqIO.parse(open("KS.119.10kb.fasta.cdhit.90", "rU"), "fasta"):
    gbidfull = record.id
    #print gbidfull
    gbid, target, _, _, _, _, cl_num, cl_type = gbidfull.split("|")[:8]
    # if gbid not in gbid_to_species.keys():
    #     f.write("%s\tNone\n" % gbidfull)
    #     print "%s\tNone" % gbidfull
    #     continue
    descr = "_".join([gbid, target, cl_type, cl_num, gbid_to_species[gbid]])
    f.write("%s\t  %s\n" % (gbidfull, descr))
    print "%s\t  %s" % (gbidfull, descr)
    #print gbid_to_species[gbid]
f.close()
